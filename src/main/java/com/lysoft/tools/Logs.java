package com.lysoft.tools;

/**
 * 日志输出代理类。<p>
 * 主要完成业务代码和日志工具间的解耦，使切换日志工具更加方便。<p>
 * 如：将Log4j改为common-logging或自己的独立实现，则只需调整该类即可。<p>
...
public class Logs {
    /**
     * 是否打印日志，true表示打印日志，false表示不打印。<p>
     ...
     * 消息所属和消息内容的分隔符
     */
    private static final String msgSplit = " (σ`∀´)σ ";
    /**
     * 该类的名称，用于识别该类的堆栈
     */
     ...
     * @param level   日志级别
     * @param message 日志消息
     * @param ste     堆栈信息。 <p> 如果不需要输出源代码信息，则只需将静态成员属性 showLocSrc设为false即可。
     */
     ...
     *
     * @param ste 堆栈信息
     * @return 调用输出日志的代码所在的类.方法.代码行的相关信息<p>
     * 如：com.MyClass 类里的 fun()方法调用了Logs.debug("test");<p>
